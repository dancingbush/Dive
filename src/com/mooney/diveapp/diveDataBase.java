...
*/
	...
	// get row nuber for a foven dive number
public int getRowNumber(int diveNumber){
		
	
	// KEY_ROWID or KEY_DVENUMBER is asking whoch rows or entries to retrun
	
			Cursor c = ourDataBase.query(DATABASE_TABLE, columns, KEY__DIVENUMBER + "=" +diveNumber, null, null, null, null);
			...
			if(c!=null){
				c.moveToFirst();
				String number= c.getString(c.getColumnIndex(KEY_ROWID));//data from column containing the loaction
				...
				// if a row is deleted teh id is deleted and nevr used again so return 1 as default, so return next valid 
				// row number using recursion
				Log.d("DATABASE 280", "gETROWNUMBER ERRPR: "+ e.getMessage().toString());
				
				
			}
			
			return 0; // if we retrun 0 soemthong went wrong, ust check thos on callback
	...
	public String getDiveNumber(long l) throws SQLException{
		...
		// KEY_ROWID is asking whoch rows or entries to retrun
